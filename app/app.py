...
# --- 1. PRIVACY & CONFIGURATION (Pulled from Azure Env Vars) ---
# This ensures no personal data is hardcoded in your Public Repo
...
def get_data_and_history():
    ...
    for blob in blobs:
        if blob.name.startswith("grades_") and blob.name.endswith(".json"):
            ...
            # Extract date for history chart: grades_2026-02-01.json -> 2026-02-01
            date_val = blob.name.split('_')[1].replace('.json', '')
            df['ReportDate'] = pd.to_datetime(date_val)
            historical_data.append(df)
            current_df = df 

    return current_df, (pd.concat(historical_data) if historical_data else pd.DataFrame())
...
if not history_df.empty and len(history_df['ReportDate'].unique()) >= 2:
    ...
    st.metric("Overall Grade Average", f"{latest_avg:.1f}%", delta=f"{delta:.1f}%")

# Payout Metrics
if current_df is not None:
    # Logic App feeds 'subject' and 'score' columns
    current_data = dict(zip(current_df['subject'], current_df['score']))
    # Logic App feeds a 'missing' column
